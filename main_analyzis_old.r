...
#********
#The frailty parameter makes very difficult to predict model responses
#the predict() function of parfm package returns only estimated frailties for different subjects
#However:
#Including gamma frailty into model do not change greatly parameter estimates
#We can use model without frailty to analyses obtained interaction, especially that we have comparable parameter significances
...
####################################################################
####################################################################
#Analysis of the interaction, frailty parameter excluded 
####################################################################
####################################################################

x=0:max(dscdane1$timesurvived)
rho=Mod_noFr["rho",1]
lambda=Mod_noFr["lamda",1]
baseline.haz=lambda*rho*x^(rho-1)
mm=model.matrix(Formula,data=dscdane1)
coef.val=as.list(Mod_noFr[,1])
  ...
  if ((length(sex)>0)&&(length(treatment)>0)&&(length(bean)==1)&&(length(mass)==1)) {
    ...
    for (beani in  seq_along(dscdane1$bean1)){
      ...
      if (sum(id)>0){
        ...
        tr.c=as.data.frame(mm)$trVirgin[id][1]
        int.c=unname(unlist(as.data.frame(mm)["sexMales:trVirgin"]))[id][1]
        haz=c(haz,list(baseline.haz*exp(coef.val$mass1*mass+
                                          coef.val$sexMales*sex.c+
    ...
    for (massi in  seq_along(dscdane1$mass1)){
      ...
      if (sum(id)>0){
        ...
        tr.c=as.data.frame(mm)$trVirgin[id][1]
        int.c=unname(unlist(as.data.frame(mm)["sexMales:trVirgin"]))[id][1]
        haz=c(haz,list(baseline.haz*exp(coef.val$mass1*massv+
                                          coef.val$sexMales*sex.c+
                                          coef.val$bean1*bean+
                                          coef.val$trVirgin*tr.c+
                                          unname(unlist(coef.val["sexMales:trVirgin"]))*int.c)))
    ...
    idF=(dscdane1$tr==treatment)&(dscdane1$sex=='Females')
    idM=(dscdane1$tr==treatment)&(dscdane1$sex=='Males')
    if (bean=='mean') bean.F=mean(dscdane1$bean1[idF]) else bean.F=bean
    if (bean=='mean') bean.M=mean(dscdane1$bean1[idM]) else bean.M=bean
    if (mass=='mean') mass.F=mean(dscdane1$mass1[idF]) else mass.F=mass
    if (mass=='mean') mass.M=mean(dscdane1$mass1[idM]) else mass.M=mass
    
    sex.c.F=as.data.frame(mm)$sexMales[idF][1]
    tr.c.F=as.data.frame(mm)$trVirgin[idF][1]
    int.c.F=unname(unlist(as.data.frame(mm)["sexMales:trVirgin"]))[idF][1]
    sex.c.M=as.data.frame(mm)$sexMales[idM][1]
    tr.c.M=as.data.frame(mm)$trVirgin[idM][1]
    int.c.M=unname(unlist(as.data.frame(mm)["sexMales:trVirgin"]))[idM][1]
    
    haz.F=baseline.haz*exp(coef.val$mass1*mass.F+
                             coef.val$sexMales*sex.c.F+
                             coef.val$bean1*bean.F+
                             coef.val$trVirgin*tr.c.F+
                             unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.F)
    haz.M=baseline.haz*exp(coef.val$mass1*mass.M+
                             coef.val$sexMales*sex.c.M+
                             coef.val$bean1*bean.M+
                             coef.val$trVirgin*tr.c.M+
                             unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.M)
    LF=sum(idF); LM=sum(idM)
    LF=LF/(LF+LM); LM=LM/(LF+LM)
    #marginal hazard
    y=(haz.F*LF*exp(-cumsum(haz.F))+haz.M*LM*exp(-cumsum(haz.M)))/(LF*exp(-cumsum(haz.F))+LM*exp(-cumsum(haz.M)))
  } else if ((length(sex)>=0)&&(length(treatment)==0)&&(length(bean)==1)&&(length(mass)==1)) {
    #Calculate mixture of the hazards
    idV=(dscdane1$sex==sex)&(dscdane1$tr=='Virgin')
    idR=(dscdane1$sex==sex)&(dscdane1$tr=='Reproducing')
    if (bean=='mean') bean.V=mean(dscdane1$bean1[idV]) else bean.V=bean
    if (bean=='mean') bean.R=mean(dscdane1$bean1[idR]) else bean.R=bean
    if (mass=='mean') mass.V=mean(dscdane1$mass1[idV]) else mass.V=mass
    if (mass=='mean') mass.R=mean(dscdane1$mass1[idR]) else mass.R=mass
    
    sex.c.V=as.data.frame(mm)$sexMales[idV][1]
    tr.c.V=as.data.frame(mm)$trVirgin[idV][1]
    int.c.V=unname(unlist(as.data.frame(mm)["sexMales:trVirgin"]))[idV][1]
    sex.c.R=as.data.frame(mm)$sexMales[idR][1]
    tr.c.R=as.data.frame(mm)$trVirgin[idR][1]
    int.c.R=unname(unlist(as.data.frame(mm)["sexMales:trVirgin"]))[idR][1]
    
    haz.V=baseline.haz*exp(coef.val$mass1*mass.V+
                             coef.val$sexMales*sex.c.V+
                             coef.val$bean1*bean.V+
                             coef.val$trVirgin*tr.c.V+
                             unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.V)
    haz.R=baseline.haz*exp(coef.val$mass1*mass.R+
                             coef.val$sexMales*sex.c.R+
                             coef.val$bean1*bean.R+
                             coef.val$trVirgin*tr.c.R+
                             unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.R)
    LV=sum(idV); LR=sum(idR)
    LV=LV/(LV+LR); LR=LR/(LV+LR)
    #marginal hazard
    y=(haz.V*LV*exp(-cumsum(haz.V))+haz.R*LR*exp(-cumsum(haz.R)))/(LV*exp(-cumsum(haz.V))+LR*exp(-cumsum(haz.R)))
  } else if ((length(sex)==0)&&(length(treatment)==0)&&(length(bean)==1)&&(length(mass)==1)) {
    #Calculate mixture of the hazards
    ...
    if (bean=='mean') bean.VF=mean(dscdane1$bean1[idVF]) else bean.VF=bean #sorry fo copy-paste ;)
    ...
    #marginal hazard
    y=(haz.VF*LVF*exp(-cumsum(haz.VF))+haz.RF*LRF*exp(-cumsum(haz.RF))+
    ...
    for (massi in  seq_along(dscdane1$mass1[idVF])){
      ...
      haz.VF=c(haz.VF,list(baseline.haz*exp(coef.val$mass1*massv+
                                              coef.val$sexMales*sex.c.VF+
                                              coef.val$bean1*bean.VF+
                                              coef.val$trVirgin*tr.c.VF+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.VF)))
    ...
    for (massi in  seq_along(dscdane1$mass1[idVM])){
      ...
      haz.VM=c(haz.VM,list(baseline.haz*exp(coef.val$mass1*massv+
                                              coef.val$sexMales*sex.c.VM+
                                              coef.val$bean1*bean.VM+
                                              coef.val$trVirgin*tr.c.VM+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.VM)))
    ...
    for (massi in  seq_along(dscdane1$mass1[idRF])){
      ...
      haz.RF=c(haz.RF,list(baseline.haz*exp(coef.val$mass1*massv+
                                              coef.val$sexMales*sex.c.RF+
                                              coef.val$bean1*bean.RF+
                                              coef.val$trVirgin*tr.c.RF+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.RF)))
    ...
    for (massi in  seq_along(dscdane1$mass1[idRM])){
      ...
      haz.RM=c(haz.RM,list(baseline.haz*exp(coef.val$mass1*massv+
                                              coef.val$sexMales*sex.c.RM+
                                              coef.val$bean1*bean.RM+
                                              coef.val$trVirgin*tr.c.RM+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.RM)))
    ...
    for (beani in  seq_along(dscdane1$bean1[idVF])){
      ...
      haz.VF=c(haz.VF,list(baseline.haz*exp(coef.val$mass1*mass.VF+
                                              coef.val$sexMales*sex.c.VF+
                                              coef.val$bean1*beanv+
                                              coef.val$trVirgin*tr.c.VF+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.VF)))
    ...
    for (beani in  seq_along(dscdane1$bean1[idVM])){
      ...
      haz.VM=c(haz.VM,list(baseline.haz*exp(coef.val$mass1*mass.VM+
                                              coef.val$sexMales*sex.c.VM+
                                              coef.val$bean1*beanv+
                                              coef.val$trVirgin*tr.c.VM+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.VM)))
    ...
    for (beani in  seq_along(dscdane1$bean1[idRF])){
      ...
      haz.RF=c(haz.RF,list(baseline.haz*exp(coef.val$mass1*mass.RF+
                                              coef.val$sexMales*sex.c.RF+
                                              coef.val$bean1*beanv+
                                              coef.val$trVirgin*tr.c.RF+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.RF)))
    ...
    for (beani in  seq_along(dscdane1$bean1[idRM])){
      ...
      haz.RM=c(haz.RM,list(baseline.haz*exp(coef.val$mass1*mass.RM+
                                              coef.val$sexMales*sex.c.RM+
                                              coef.val$bean1*beanv+
                                              coef.val$trVirgin*tr.c.RM+
                                              unname(unlist(coef.val["sexMales:trVirgin"]))*int.c.RM)))
...
#survivorships as Mariusz did for survreg
#############################################################################################
#NOT FOR PUBLICATION
